...
<script>
  ...
  let resources = [];
  let api = "https://api.apispreadsheets.com/data/1571/";
...
fetch(api).then(res=>{
 if (res.status === 200){
  ...
  res.json().then(data=>{
    ...
    localStorage.setItem("atadgib", JSON.stringify(ass));
    resources = JSON.parse(localStorage.getItem("atadgib"));
  ...
  if (localStorage.getItem("atadgib") === null) {
    ...
    <div class="font-inter flex flex-grow h-auto justify-center items-center py-10">
        <div class="text-center">
          ...
          <p class="textBox text-lg mt-2 m-5">
            <strong>{name}</strong> is a field that treats ways to analyze, systematically extract information from, or otherwise deal with <br/>data sets that are too large or complex to be dealt with by traditional data-processing application software. <a class="text-blue-700" href="https://en.wikipedia.org/wiki/Big_data">(wiki.)</a><br/>
          ...
          <button class="bg-white hover:bg-gray-100 text-gray-800 font-semibold py-2 px-4 border border-gray-400 rounded shadow" on:click={()=>{ resources = JSON.parse(localStorage.getItem("atadgib")); }}>
            All
          </button>
        </div>
    </div>

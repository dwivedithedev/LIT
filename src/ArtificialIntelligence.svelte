...
<script>
  ...
  let name="Artificial Intelligence";
  
  let resources = [];

  let api = "https://api.apispreadsheets.com/data/1568/";
...
fetch(api).then(res=>{
 if (res.status === 200){
  ...
  res.json().then(data=>{
    ...
    localStorage.setItem("ecnegilletnIlaicifitrA", JSON.stringify(ass));
    resources = JSON.parse(localStorage.getItem("ecnegilletnIlaicifitrA"));
  ...
  if (localStorage.getItem("ecnegilletnIlaicifitrA") === null) {
    ...
    <div class="font-inter flex flex-grow h-auto justify-center items-center py-10">
        <div class="text-center">
          ...
          <p class="textBox text-lg mt-2 m-5">
            <strong>{name}</strong>, also called machine intelligence, is intelligence demonstrated by machines, unlike the natural intelligence displayed by humans and animals. <a class="text-blue-700" href="https://en.wikipedia.org/wiki/Artificial_intelligence">(wiki.)</a><br/>
            A.I consists of lot of topics such as NLP, Neural Networks etc. It is recommended that you have a good experience in ML + DL before you dive into Artificial Intelligence.<br/>
          ...
          <button class="bg-white hover:bg-gray-100 text-gray-800 font-semibold py-2 px-4 border border-gray-400 rounded shadow" on:click={()=>{ resources = JSON.parse(localStorage.getItem("ecnegilletnIlaicifitrA")); }}>
            All
          </button>
        </div>
    </div>

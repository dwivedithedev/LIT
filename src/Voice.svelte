...
<script>
  ...
  let resources = [];
  let api = "https://api.apispreadsheets.com/data/1633/";
...
fetch(api).then(res=>{
 if (res.status === 200){
  ...
  res.json().then(data=>{
    ...
    localStorage.setItem("eciov", JSON.stringify(ass));
    resources = JSON.parse(localStorage.getItem("eciov"));
  ...
  if (localStorage.getItem("eciov") === null) {
    ...
    <div class="font-inter flex flex-grow h-auto justify-center items-center py-10">
        <div class="text-center">
          ...
          <p class="textBox text-lg mt-2 m-5">
            <strong>{name}</strong> is an interdisciplinary subfield of computer science and computational linguistics that  <br/>develops methodologies and technologies that enable the recognition and translation of spoken language into text by computers. <a class="text-blue-700" href="https://en.wikipedia.org/wiki/Automation">(wiki.)</a><br/>
          ...
          <button class="bg-white hover:bg-gray-100 text-gray-800 font-semibold py-2 px-4 border border-gray-400 rounded shadow" on:click={()=>{ resources = JSON.parse(localStorage.getItem("eciov")); }}>
            All
          </button>
        </div>
    </div>

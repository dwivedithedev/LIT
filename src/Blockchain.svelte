...
<script>
  ...
  let resources = [];
  let api = "https://api.apispreadsheets.com/data/1584/";
...
fetch(api).then(res=>{
 if (res.status === 200){
  ...
  res.json().then(data=>{
    ...
    localStorage.setItem("niahckcolb", JSON.stringify(ass));
    resources = JSON.parse(localStorage.getItem("niahckcolb"));
  ...
  if (localStorage.getItem("niahckcolb") === null) {
    ...
    <div class="font-inter flex flex-grow h-auto justify-center items-center py-10">
        <div class="text-center">
          ...
          <p class="textBox text-lg mt-2 m-5">
            <strong>{name}</strong> originally block chain, is a growing list of records, called blocks, that are linked using cryptography. Each block contains a cryptographic hash of the previous block,a timestamp, and transaction data (generally represented as a Merkle tree). <a class="text-blue-700" href="https://en.wikipedia.org/wiki/Blockchain">(wiki.)</a><br/>
          ...
          <button class="bg-white hover:bg-gray-100 text-gray-800 font-semibold py-2 px-4 border border-gray-400 rounded shadow" on:click={()=>{ resources = JSON.parse(localStorage.getItem("niahckcolb")); }}>
            All
          </button>
        </div>
    </div>

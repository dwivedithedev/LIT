...
<style>
  ...
  <script>
    ...
    let api = "https://api.apispreadsheets.com/data/1393/";
    let ass=[];
    let resBeginners = [];
  let resIntermediate = [];
  let resAdvanced = [];
...
fetch(api).then(res=>{
 if (res.status === 200){
  ...
  res.json().then(data=>{
    ...
    localStorage.setItem("vedemag", JSON.stringify(ass));
    resources = JSON.parse(localStorage.getItem("vedemag"));
  ...
  if (localStorage.getItem("vedemag") === null) {
  ...
  <div>
      ...
      <div class="font-inter flex flex-grow h-auto justify-center items-center py-10">
          <div class="text-center">
            ...
            <button class="bg-white hover:bg-gray-100 text-gray-800 font-semibold py-2 px-4 border border-gray-400 rounded shadow" on:click={()=>{ resources = JSON.parse(localStorage.getItem("vedemag")); }}>
              All
            </button>
          </div>
      </div>

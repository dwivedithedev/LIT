...
<script>
  ...
  let resources = [];
  let api = "https://api.apispreadsheets.com/data/1630/";
...
fetch(api).then(res=>{
 if (res.status === 200){
  ...
  res.json().then(data=>{
    ...
    localStorage.setItem("noitamotua", JSON.stringify(ass));
    resources = JSON.parse(localStorage.getItem("noitamotua"));
  ...
  if (localStorage.getItem("noitamotua") === null) {
    ...
    <div class="font-inter flex flex-grow h-auto justify-center items-center py-10">
        <div class="text-center">
          ...
          <p class="textBox text-lg mt-2 m-5">
            <strong>{name}</strong> is the technology by which a process or procedure is performed with minimal human assistance. <a class="text-blue-700" href="https://en.wikipedia.org/wiki/Automation">(wiki.)</a><br/>
            This list covers learning about Automation with IoT, Raspberry Pi, Arduino etc. for you to get started in this field. <br/>Here is the list of top resources you can access to get started with {name}.
          </p>
          ...
          <button class="bg-white hover:bg-gray-100 text-gray-800 font-semibold py-2 px-4 border border-gray-400 rounded shadow" on:click={()=>{ resources = JSON.parse(localStorage.getItem("noitamotua")); }}>
            All
          </button>
        </div>
    </div>

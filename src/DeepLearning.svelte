...
<style>
  ...
  <script>
    ...
    let resources = [];
    let name="Deep Learning";
    let api = "https://api.apispreadsheets.com/data/1556/";
...
fetch(api).then(res=>{
 if (res.status === 200){
  ...
  res.json().then(data=>{
    ...
    localStorage.setItem("gninraelpeed", JSON.stringify(ass));
    resources = JSON.parse(localStorage.getItem("gninraelpeed"));
  ...
  if (localStorage.getItem("gninraelpeed") === null) {
  ...
  <div>
      ...
      <div class="font-inter flex flex-grow h-auto justify-center items-center py-10">
          <div class="text-center">
            ...
            <button class="bg-white hover:bg-gray-100 text-gray-800 font-semibold py-2 px-4 border border-gray-400 rounded shadow" on:click={()=>{ resources = JSON.parse(localStorage.getItem("gninraelpeed")); }}>
              All
            </button>
          </div>
      </div>

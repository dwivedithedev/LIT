...
<script>
  ...
  let name="Augmented Reality";
  
  let resources = [];

  let api = "https://api.apispreadsheets.com/data/1374/";
    ...
    fetch(api).then(res=>{
     if (res.status === 200){
      ...
      res.json().then(data=>{
        ...
        localStorage.setItem("ytilaerdetnemgua", JSON.stringify(ass));
        resources = JSON.parse(localStorage.getItem("ytilaerdetnemgua"));
  ...
  onMount(()=>{
    ...
    if (localStorage.getItem("ytilaerdetnemgua") === null) {
    ...
    <div class="font-inter flex flex-grow h-auto justify-center items-center py-10">
        <div class="text-center">
          ...
          <p class="textBox text-lg mt-2 m-5">
            <strong>Augmented reality (AR)</strong> is an interactive experience of a real-world environment 
              where the objects that reside in the real world are enhanced by computer-generated perceptual information, 
              sometimes across multiple sensory modalities, including visual, auditory, haptic, somatosensory and olfactory. <a class="text-blue-700" href="https://en.wikipedia.org/wiki/Augmented_reality">(wiki.)</a><br/>
              Here is the list of top resources you can access to get started with Augmented Reality.
          </p>
          ...
          <button class="bg-white hover:bg-gray-100 text-gray-800 font-semibold py-2 px-4 border border-gray-400 rounded shadow" on:click={()=>{ resources = JSON.parse(localStorage.getItem("ytilaerdetnemgua")); }}>
            All
          </button>
        </div>
    </div>

...
<style>
  ...
  <script>
    ...
    let resources = [];
    let name="Internet of Things";
    let api = "https://api.apispreadsheets.com/data/1405/";
    let ass = [];
  let resBeginners = [];
  let resIntermediate = [];
  let resAdvanced = [];
...
fetch(api).then(res=>{
 if (res.status === 200){
  ...
  res.json().then(data=>{
    ...
    localStorage.setItem("toi", JSON.stringify(ass));
    resources = JSON.parse(localStorage.getItem("toi"));
  ...
  if (localStorage.getItem("toi") === null) {
      ...
      <div class="font-inter flex flex-grow h-auto justify-center items-center py-10">
          <div class="text-center">
            ...
            <p class="textBox text-lg mt-2 m-5">
              <strong>Internet of Things </strong> describes the network of physical objects — “things” — that are embedded with sensors, software, and 
              other technologies for the purpose of connecting and exchanging data with other devices and systems over the internet. <a class="text-blue-700" href="https://en.wikipedia.org/wiki/Internet_of_things">(wiki.)</a><br/>
            ...
            <button class="bg-white hover:bg-gray-100 text-gray-800 font-semibold py-2 px-4 border border-gray-400 rounded shadow" on:click={()=>{ resources = JSON.parse(localStorage.getItem("toi")); }}>
              All
            </button>
          </div>
      </div>

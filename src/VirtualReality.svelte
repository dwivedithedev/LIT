...
<style>
  ...
  <script>
    ...
    let resources = [];
    let name="Virtual Reality";
    let api = "https://api.apispreadsheets.com/data/1383/";
    let ass = [];
  let resBeginners = [];
  let resIntermediate = [];
  let resAdvanced = [];
...
fetch(api).then(res=>{
 if (res.status === 200){
  ...
  res.json().then(data=>{
    ...
    localStorage.setItem("ytilaerlautriv", JSON.stringify(ass));
    resources = JSON.parse(localStorage.getItem("ytilaerlautriv"));
  ...
  if (localStorage.getItem("ytilaerlautriv") === null) {
      ...
      <div class="font-inter flex flex-grow h-auto justify-center items-center py-10">
          <div class="text-center">
            ...
            <p class="textBox text-lg mt-2 m-5">
              <strong>Virtual reality (VR)</strong>  is a simulated experience that can be similar to or completely different from the real world.<br>
                Applications of virtual reality can include entertainment and educational purposes. <a class="text-blue-700" href="https://en.wikipedia.org/wiki/Virtual_reality">(wiki.)</a><br/>
                Here is the list of top resources you can access to get started with Virtual Reality.
            </p>
            ...
            <button class="bg-white hover:bg-gray-100 text-gray-800 font-semibold py-2 px-4 border border-gray-400 rounded shadow" on:click={()=>{ resources = JSON.parse(localStorage.getItem("ytilaerlautriv")); }}>
              All
            </button>
          </div>
      </div>
